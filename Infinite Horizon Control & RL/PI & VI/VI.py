...
#                     if v < self.v_max and v > -1*self.v_max:
    ...
    def value_itr (self,n1,n2,n_u,dt):
        #used to plot
            ...
            for i in range(n1):
                for j in range(n2):
                    ...
                    for k in range(n_u):
                        ...
                        cost[k] = l + self.gamma*(np.sum(self.oldV*self.p_f[i,j,k]))
                    self.V[i,j] = np.min(cost)
                    self.policy[i,j] = self.u[np.int(np.argmin(cost))]
            itr += 1
            error = np.sum(np.absolute(self.oldV - self.V))
            print([itr, error])
            self.oldV = self.V.copy()
        print('Value Iteration Finished')
        imgplot = plt.imshow(self.V)
        plt.colorbar()
        plt.show()
        plt.savefig('value_VI.png')
        
        imgplot = plt.imshow(self.policy)
        plt.colorbar()
        plt.show()
        plt.savefig('policy_VI.png')
    ...
    def start(self):
        #value_iteration
        self.value_itr(self.n1,self.n2,self.n_u,self.dt_itr)
...
# import matplotlib.pyplot as plt
# s1 = np.asarray(animation.v_s1)
# s2 = np.asarray(animation.v_s2)
# s3 = np.asarray(animation.v_s3)
# print(s1.shape)
# t = np.arange(s1.shape[0])

# idx1 = animation.ss1[0]
# idx2 = animation.ss1[1]
# plt.figure(3)
# plt.plot(t,s1)
# plt.title('V('+str([animation.s1[idx1],animation.s2[idx2]])+')')
# plt.savefig('p4_VI_s1.png')
# plt.show()
# plt.figure(4)
# idx1 = animation.ss2[0]
# idx2 = animation.ss2[1]
# plt.plot(t,s2)
# plt.title('V('+str([animation.s1[idx1],animation.s2[idx2]])+')')
# plt.savefig('p4_VI_s2.png')
# plt.show()
# plt.figure(5)
# idx1 = animation.ss3[0]
# idx2 = animation.ss3[1]
# plt.plot(t,s3)
# plt.title('V('+str([animation.s1[idx1],animation.s2[idx2]])+')')
# plt.savefig('p4_VI_s3.png')
# plt.show()
# plt.figure(6)
# plt.imshow(animation.V)
# plt.colorbar()
# plt.title('VI-Value')
# plt.savefig('p4_vi_policy.png')
# plt.show()
# plt.figure(7)
